...
Use with caution if you're using other mod managers like Vortex or MO2, as they may conflict with this tool.
Running Vortex after using this tool will likely disrupt your load order.

This tool provides a load order editor for managing mods in Starfield.
It automatically checks the `ContentCatalog.txt` file when launched and has options to repair the file.
...
- Multi-select mods with `Ctrl+Click` or `Shift+Click` for batch actions.
...
- **Block Mods**: Block specific mods from activation. Right-click menu
...
- View LOOT groups if LOOT is installed. Enable this via **View -> Columns -> Group**.
- Launch the game directly from the tool (Steam, MS Store, or SFSE versions).

### Profiles
- Create and switch between mod profiles for different save games.
- Profiles ensure your load order is restored after being affected by other apps.
...
## How Starfield Handles Mods
- Mods are listed in `Plugins.txt` under `%localappdata%\Starfield`.
- Active mods are marked with `*`, while inactive mods are unmarked.
- Example:

### Reset Options
- **File -> Reset/Delete Loose File Folders**: 
  ...
  - Use this option to clean up leftover files from uninstalled mods. Avoid using it if you want to keep loose file mods.
- **File -> Reset/Reset Everything**: This is almost like a clean game install but keeps your mods that are in .esm and .ba2 format.
  ...
  - Checks for and optionally deletes left over `.ba2` archive files without corresponding `.esm` files.
  - Resets changes made to the Starfield Documents folder by Vortex.
